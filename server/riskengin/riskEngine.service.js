...
export const analyzeLoginRisk = async (userId, ipAddress, userAgent) => {
  try {
    ...
    console.log(
      `[RiskEngine] Result: ${response.data.status} (Score: ${response.data.riskScore})`
    );
    return response.data;
  } catch (error) {
    console.error("[RiskEngine] Connection Failed:", error.message);
    console.error("❌ CRITICAL: Could not connect to Spring Boot!");
    if (error.code === "ECONNREFUSED") {
      console.error("👉 Check if Spring Boot is actually running on Port 8081");
    } else {
